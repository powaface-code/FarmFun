...
export function openPrestige() {
  ...
  document.getElementById('modal-seeds').textContent = fmtNum(prestigeSeeds)+' 🌿';
...
export function doPrestige() {
  ...
  closePrestige();
  notify(`✨ Prestiž! +${fmtNum(prestigeSeeds)} 🌿 Nový bonus: ×${fmtNum(1+state.totalSeeds*0.1)}`, 'purple');
...
export function updateStats() {
  ...
  document.getElementById('st-seeds').textContent   = fmtNum(state.totalSeeds)+' 🌿';
  document.getElementById('st-prestige').textContent = state.prestigeCount;
  const seeds = calcPrestigeSeeds();
  document.getElementById('st-seeds-preview').textContent = `Získáš: ${fmtNum(seeds)} 🌿`;
  document.getElementById('st-prestige-mult').textContent = '×'+fmtNum(1+state.totalSeeds*0.1);
}
